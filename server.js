...
app.use(cors({
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'password', 'x-contractor-token']
}));
...
app.post('/api/leads', asyncHandler(async (req, res) => {
  ...
  const result = await pool.query(
    ...
    [contractor_name, contractor_company, contractor_email,
     building_owner_name, building_owner_email, building_type,
     monthly_bill, annual_savings, payback_years, units_recommended,
     proposal_generated_at || new Date().toISOString(), zip_code || null]
  );
...
app.post('/api/leads/consumer', asyncHandler(async (req, res) => {
  const { consumer_name, consumer_email, consumer_phone, building_type, monthly_bill, zip_code } = req.body;

  const bill = parseFloat(monthly_bill) || 0;
  const savingsPct = building_type === 'residential' ? 0.32 : building_type === 'commercial' ? 0.28 : 0.30;
  ...
  const result = await pool.query(
    ...
    [consumer_name, consumer_email, building_type, bill, annual_savings,
     payback_years, units_recommended, zip_code || null, consumer_name, consumer_phone || null]
  );
  ...
  fetch('https://formspree.io/f/mlgooqew', {
    ...
    body: JSON.stringify({
      ...
      name: consumer_name, email: consumer_email, phone: consumer_phone || 'Not provided',
      building_type, monthly_bill: `$${bill}`, zip_code: zip_code || 'Not provided',
      annual_savings: `$${annual_savings}`, payback: `${payback_years} years`, source: 'Consumer Savings Page',
    })
  }).catch(() => {});
...
app.get('/api/portal/leads', asyncHandler(async (req, res) => {
  ...
  if (password !== process.env.PORTAL_PASSWORD) return res.status(401).json({ error: 'Unauthorized' });
...
app.get('/api/portal/stats', asyncHandler(async (req, res) => {
  ...
  res.json({ summary: stats.rows[0], by_building_type: byType.rows, recent_proposals: recent.rows });
}));
...
app.get('/api/portal/map', asyncHandler(async (req, res) => {
  ...
  const data = await pool.query(`
    SELECT zip_code, COUNT(*) as proposal_count,
    ...
    GROUP BY zip_code ORDER BY proposal_count DESC`);
  res.json(data.rows);
}));
...
app.get('/api/contractor/leads', asyncHandler(async (req, res) => {
  const contractor_id = await verifyToken(req.headers['x-contractor-token']);
...
async function verifyToken(token) {
  ...
  try {
    ...
    const r = await pool.query(`SELECT id FROM contractors WHERE id = $1 AND is_active = true`, [parseInt(id)]);
